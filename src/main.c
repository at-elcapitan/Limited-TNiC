...
void on_interaction(struct discord *bot,
                    const struct discord_interaction *event) {
#ifdef DEBUG
  if (app.config->allowedBotAdmin != 0 &&
      event->member->user->id != app.config->allowedBotAdmin) {
    tnic_sendErrorEmbed(app, event, "#eDEB", "You are not allowed to use bot");
    return;
  }
#endif
...
void on_coglink_track_end(struct coglink_client *c_client,
                          struct coglink_node *node,
...
enum tnic_errorTypes loadApplicationConfig(tnic_application app) {
  ...
  if (value.start != NULL) {
    ...
    log_info("[TNiC] Loaded game name from config: \"%s\"",
             app.config->botGameName);
  ...
  if (value.start != NULL) {
    ...
    log_info("[TNiC] Loaded admin_debug_id from config: %llu",
             app.config->allowedBotAdmin);
  } else {
    app.config->allowedBotAdmin = 0;
    log_info("[TNiC] admin_debug_id not set, commands won't be locked",
             app.config->allowedBotAdmin);
  }
#endif
...
int main(void) {
  ...
  puts("AT PROJECT Limited, 2021 - 2025; ATNiC-v0.1.0");
  puts("Product licensed by GPLv3, file `LICENSE`");
  puts("This is a prototype version and should not be used in production "
       "environments");
  ...
  app.playlistController =
      (tnic_playlist_controller *)malloc(sizeof(tnic_playlist_controller));
  ...
  // Prepearing coglink
  struct coglink_nodes nodes = {.array = (struct coglink_node[]){{
                                    .name = "Node 1",
                                    .hostname = "127.0.0.1",
                                    .port = 2333,
  ...
  app.client->events = &(struct coglink_events){
      .on_ready = &on_coglink_ready, .on_track_end = &on_coglink_track_end};
