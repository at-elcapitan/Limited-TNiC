...
tnic_errnoReturn playlist_addTrack(tnic_playlist *playlist, tnic_track *track) {
  tnic_track **newTracksArr =
      (tnic_track **)malloc(sizeof(tnic_track *) * (playlist->size + 1));

  if (!newTracksArr) {
    log_debug("[TNIC/Errno] Errno: tnic_IS_NULL, position: %d, at %s", __LINE__,
              __FILE__);
...
void playlist_changeState(tnic_playlist *playlist,
                          const enum tnic_playlistStates state) {
...
tnic_errnoReturn playlist_getTrack(tnic_playlist *playlist,
