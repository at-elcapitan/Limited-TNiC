 ...
 */
void tnic_sendErrorEmbed(tnic_application app,
                         const struct discord_interaction *event,
                         char *errstring, char *message) {
  struct discord_embed_footer footer = {
      .text = errstring, .icon_url = NULL, .proxy_icon_url = NULL};

  struct discord_embed embed = {
      .color = 0xFF0000, .title = message, .footer = &footer};
  ...
  discord_create_interaction_response(app.bot, event->id, event->token, &params,
                                      NULL);
}

void tnic_sendInfoEmbed(tnic_application app,
                        const struct discord_interaction *event,
                        char *message) {
  struct discord_interaction_response params = {
      .type = DISCORD_INTERACTION_CHANNEL_MESSAGE_WITH_SOURCE,
      .data = &(struct discord_interaction_callback_data){
          .content = message, .flags = DISCORD_MESSAGE_EPHEMERAL}};
